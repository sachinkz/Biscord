    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:neutral-400 space-y-[2px]">
                ...
                {isAdmin && (
                    <DropdownMenuItem onClick={()=>onOpen("editServer",{server})} className="text-zinc-200 px-3 py-2 text-sm cursor-pointer ">
                        Server Settings
                        <Settings className="h-4 w-4 ml-auto" />
                    </DropdownMenuItem>
                )}
                {isAdmin && (
                    <DropdownMenuItem onClick={()=>onOpen("members",{server})} className="text-zinc-200 px-3 py-2 text-sm cursor-pointer ">
                        Manage Members
                        <Users className="h-4 w-4 ml-auto" />
                    </DropdownMenuItem>
                )}
                {isModerator && (
                    <DropdownMenuItem onClick={()=>onOpen("createChannel",{})} className="text-zinc-200 px-3 py-2 text-sm cursor-pointer ">
                        Create channel
